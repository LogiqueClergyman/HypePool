...
/* ── Mock card ────────────────────────────────────── */
function MockCard() {
  const [timer, setTimer] = useState({ h: 14, m: 22, s: 47 });
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#080808] border border-white/[0.08] overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.05] bg-white/[0.015]">
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-black text-muted-foreground">
            ...
            <span className="tabular-nums">{fmt(timer.h)}:{fmt(timer.m)}:{fmt(timer.s)}</span>
          </div>
        </div>

        {/* Thumbnail */}
        <div className="relative w-full aspect-video overflow-hidden">
          {/* Gradient thumbnail that looks like a YouTube video */}
          ...
          <div className="absolute top-3 left-3 bg-black/70 backdrop-blur-sm border border-white/10 px-2.5 py-1">
            <span className="text-[8px] font-black text-white uppercase tracking-wider">MR. BEAST</span>
          </div>
        </div>
        ...
        <div className="px-5 pt-4 pb-3">
          <p className="text-[11px] font-black text-white uppercase tracking-tight leading-tight">
            WILL THIS VIDEO HIT <span className="text-primary">100M VIEWS</span> IN 48 HOURS?
          </p>
        </div>
        ...
        <div className="px-5 pb-4 space-y-2.5">
          <div>
            <div className="flex justify-between mb-1.5">
              ...
              <span className="text-[9px] font-black text-primary">68%</span>
            </div>
            <LiveBar pct={68} color="#00FF85" />
          </div>
          <div>
            <div className="flex justify-between mb-1.5">
              <span className="text-[9px] font-black text-muted-foreground tracking-widest">NO</span>
              <span className="text-[9px] font-black text-muted-foreground">32%</span>
            </div>
            <LiveBar pct={32} color="rgba(255,255,255,0.25)" />
          </div>
        </div>
        ...
        <div className="px-5 py-3 border-t border-white/[0.05] flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div>
              ...
              <p className="text-xs font-black text-white italic">284K XLM</p>
            </div>
            <div className="flex items-center gap-1.5">
              <Users className="w-3 h-3 text-muted-foreground" />
              <p className="text-xs font-black text-white italic">1,429</p>
            </div>
          </div>
          <motion.button
      ...
      >
        ...
        <span className="text-[9px] font-black text-primary tracking-widest">+12.4% ODDS SHIFT</span>
      </motion.div>
      ...
      >
        ...
        <span className="text-[8px] font-black text-muted-foreground tracking-widest">POOL GROWING</span>
        <span className="text-[10px] font-black text-white">284,100 XLM</span>
      </motion.div>
    </motion.div>
  );
}
            ...
            <div>
              ...
              <FadeUp delay={0.52}>
                <div className="grid grid-cols-3 divide-x divide-white/[0.06] border border-white/[0.06]">
                  {[
                    { label: "TOTAL VOLUME", to: 2400000, suffix: " XLM" },
                    { label: "ACTIVE MARKETS", to: 148, suffix: "" },
                    { label: "BETTORS", to: 8920, suffix: "" },
                  ].map((s, i) => (
