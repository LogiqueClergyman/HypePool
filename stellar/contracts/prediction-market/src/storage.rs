...
use crate::types::{MarketConfig, MarketState};
...
pub fn get_yes_bet(env: &Env, user: &Address) -> i128 {
    env.storage().persistent().get(&DataKey::YesBet(user.clone())).unwrap_or(0)
}

pub fn set_yes_bet(env: &Env, user: &Address, amount: i128) {
    let key = DataKey::YesBet(user.clone());
    env.storage().persistent().set(&key, &amount);
    env.storage().persistent().extend_ttl(&key, TTL_BUMP, TTL_BUMP);
}

pub fn get_no_bet(env: &Env, user: &Address) -> i128 {
    env.storage().persistent().get(&DataKey::NoBet(user.clone())).unwrap_or(0)
}

pub fn set_no_bet(env: &Env, user: &Address, amount: i128) {
